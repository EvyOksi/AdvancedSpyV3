...
function TouchControls:SetupTouchControls()
    ...
    local function handleGesture(touchPositions, deltaTime)
        if #touchPositions >= 3 then
            -- Three finger gesture for instant minimize
            self:MinimizeWindow()
        elseif #touchPositions == 2 then
            -- Enhanced pinch detection with rotation
            local delta = (touchPositions[1] - touchPositions[2]).magnitude
            local angle = math.atan2(touchPositions[1].Y - touchPositions[2].Y,
                                   touchPositions[1].X - touchPositions[2].X)
            self:HandlePinchRotate(delta, angle)
        end
    end
